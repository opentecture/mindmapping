...
MJL.getMenuJsonLines = function() {
	...
	<details id=MJLdet ontoggle=MJL.setMenuJsonLines(); >
		...
		<summary>Button for every JSON line in file MJL</summary>
...
MJL.setMenuJsonLines = function( bookmarks = BM.jsonLines ){
	...
	for ( let mark of bookmarks) {
		...
		const index = BM.jsonLines.indexOf( mark );
...
MJL.filterBookmarks = function ( input ) {
	...
	if ( str === "" ) {
		...
		MJL.bookmarks = BM.lines.map( line => JSON.parse( line ) )
